...
function formatTimestamp(ts: string, isIntraday: boolean): string {
  ...
  if (isIntraday) {
    // For intraday: show time with date
...
export default function SpreadChart({ data, symbol1, symbol2, height = 300 }: SpreadChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    const mean = data.spread.reduce((a, b) => a + b, 0) / data.spread.length;
    ...
    return {
      ...
      datasets: [
        ...
        {
          label: 'Mean',
          data: Array(data.timestamps.length).fill(mean),
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Box>
          ...
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>
            Price differential over time {isIntraday ? '(Intraday)' : '(Daily)'}
          </Typography>
        </Box>
        <Box sx={{ textAlign: 'right' }}>
          <Typography
            ...
            sx={{
              fontWeight: 700,
              color: currentSpread > mean ? theme.palette.error.main : theme.palette.success.main,
            }}
          >
            {currentSpread?.toFixed(2) || 'N/A'}
          </Typography>
          <Typography variant="caption" sx={{ color: 'text.secondary' }}>
            Current Spread (Mean: {mean.toFixed(2)})
          </Typography>
        </Box>
      </Box>
      <Box sx={{ height }}>
